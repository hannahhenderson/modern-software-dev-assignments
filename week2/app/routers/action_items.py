from __future__ import annotations

from typing import Any

from fastapi import APIRouter, HTTPException

from .. import db
from ..services.extract import (
    extract_action_items_unified,
    extract_with_ollama_detailed,
    extract_with_ollama_simple,
)

router = APIRouter(prefix="/action-items", tags=["action-items"])


@router.post("/extract")
def extract(payload: dict[str, Any]) -> dict[str, Any]:
    text = str(payload.get("text", "")).strip()
    if not text:
        raise HTTPException(status_code=400, detail="text is required")

    note_id: int | None = None
    if payload.get("save_note"):
        note_id = db.insert_note(text)

    items = extract_action_items_unified(text)
    ids = db.insert_action_items(items, note_id=note_id)
    return {"note_id": note_id, "items": [{"id": i, "text": t} for i, t in zip(ids, items, strict=False)]}


@router.post("/extract-llm")
def extract_llm(payload: dict[str, Any]) -> dict[str, Any]:
    text = str(payload.get("text", "")).strip()
    if not text:
        raise HTTPException(status_code=400, detail="text is required")

    # Get the extraction method from payload, default to simple
    method = payload.get("method", "simple")
    
    note_id: int | None = None
    if payload.get("save_note"):
        note_id = db.insert_note(text)

    # Use the specified LLM method
    if method == "detailed":
        items = extract_with_ollama_detailed(text)
    else:  # default to simple
        items = extract_with_ollama_simple(text)
    
    ids = db.insert_action_items(items, note_id=note_id)
    return {"note_id": note_id, "items": [{"id": i, "text": t} for i, t in zip(ids, items, strict=False)], "method": method}


@router.get("")
def list_all(note_id: int | None = None) -> list[dict[str, Any]]:
    rows = db.list_action_items(note_id=note_id)
    return [
        {
            "id": r["id"],
            "note_id": r["note_id"],
            "text": r["text"],
            "done": bool(r["done"]),
            "created_at": r["created_at"],
        }
        for r in rows
    ]


@router.post("/{action_item_id}/done")
def mark_done(action_item_id: int, payload: dict[str, Any]) -> dict[str, Any]:
    done = bool(payload.get("done", True))
    db.mark_action_item_done(action_item_id, done)
    return {"id": action_item_id, "done": done}


